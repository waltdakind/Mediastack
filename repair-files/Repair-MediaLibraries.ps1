...
# -----------------------------------------------------------------------------
# PHASE 4: API CREDENTIAL DISCOVERY & SEERR SYNCHRONIZATION
# -----------------------------------------------------------------------------
Write-Host "`n[PHASE 4/6] Re-Aligning Service API Keys & Integrations (Seerr/Jellyseerr)..." -ForegroundColor Yellow
...
# Auto-discover Seerr key from settings.json if present
$seerrSettingsPaths = @(
    (Join-Path $ConfigDir "seerr\settings.json"),
    (Join-Path $ConfigDir "jellyseerr\settings.json"),
    (Join-Path $BaseDir "config\seerr\settings.json"),
    (Join-Path $BaseDir "config\jellyseerr\settings.json")
)
foreach ($sp in $seerrSettingsPaths) {
    if (Test-Path $sp) {
        try {
            $parsed = Get-Content $sp -Raw -Encoding UTF8 | ConvertFrom-Json
            if ($parsed.main.apiKey) {
                $jseerrKey = $parsed.main.apiKey
                break
            }
        } catch {}
    }
