...
if (-not $DryRun) {
    $dbBackupRes = Backup-MediaStackDatabasesPreSync -ConfigDir (Join-Path $BaseDir "config") -OperationTag "PRE_PUSH_VOLTAIREDEUX"
...
if (-not $SkipPortTest) {
    ...
    $portScript = if (Test-Path (Join-Path $BaseDir "test-files\Test-MediaStackProxyAndPorts.ps1")) { Join-Path $BaseDir "test-files\Test-MediaStackProxyAndPorts.ps1" } else { Join-Path $BaseDir "Test-MediaStackProxyAndPorts.ps1" }
...
if (-not $DryRun) {
    if ($gitStatus) {
        Write-Host "  â€¢ Staging modified and untracked files into Git index..." -ForegroundColor DarkCyan
        git add -A
        
        $fullCommitMsg = "feat(cluster): [VoltaireDeux -> VoltaireUn] $Message ($fileTag)"
        Write-Host ("  â€¢ Creating Git commit: {0}" -f $fullCommitMsg) -ForegroundColor DarkCyan
        git commit -m "$fullCommitMsg" 2>&1 | Out-Null
    } else {
        Write-Host "  â€¢ Working tree is clean. No new Git changes to commit." -ForegroundColor DarkGray
    }
    ...
    if ($hasRemote) {
        Write-Host "  â€¢ Pushing changes to remote repository (origin $gitBranch)..." -ForegroundColor DarkCyan
