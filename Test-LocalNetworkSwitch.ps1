...
function Measure-PingStats {
    ...
    if ($received -gt 0) {
        ...
        Write-Host ("  • {0,-28} (IP: {1,-14}) -> Avg: {2}ms | Min: {3}ms | Max: {4}ms | Jitter: {5}ms | Loss: {6}%" -f $Label, $IP, $avg, $min, $max, $jitter, $loss) -ForegroundColor $color
        return [PSCustomObject]@{ Label = $Label; IP = $IP; Avg = $avg; Min = $min; Max = $max; Jitter = $jitter; Loss = $loss; Status = "OK" }
    } else {
        Write-Host ("  • {0,-28} (IP: {1,-14}) -> 100% PACKET LOSS (Unreachable or Blocked by Switch/Firewall)" -f $Label, $IP) -ForegroundColor Red
...
foreach ($m in $mtuSizes) {
    ...
    Write-Host ("  • {0,-36} -> Buffer: {1,4} bytes -> {2}" -f $m.Label, $m.Size, $statusText) -ForegroundColor $color
...
Write-Host ("  • mDNS Hostname Resolution (voltaireun.local) : {0}" -f $mdnsText) -ForegroundColor $(if ($mdnsOk) { "Green" } else { "Yellow" })
...
Write-Host ("  • Windows File Sharing / SMB Transport        : {0}" -f $smbText) -ForegroundColor $(if ($smbCheck.TcpTestSucceeded) { "Green" } else { "DarkGray" })
