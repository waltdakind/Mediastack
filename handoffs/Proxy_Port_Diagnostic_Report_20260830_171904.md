...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Caddy Gateway HTTP** | 80 | caddy | INGRESS | **YES** | FAILING | 1232 ms | Container 'caddy' is stopped / not created. |
| **Caddy Gateway HTTPS** | 443 | caddy | INGRESS | **YES** | FAILING | 1215 ms | Container 'caddy' is stopped / not created. |
| **API Gateway REST** | 3000 | api-gateway | API | **YES** | FAILING | 1207 ms | Container 'api-gateway' is stopped / not created. |
| **Jellyfin Media Server** | 8096 | jellyfin | STREAMING | **YES** | FAILING | 1203 ms | Container 'jellyfin' is stopped / not created. |
| **Sonarr TV Automation** | 8989 | sonarr | AUTOMATION | **YES** | FAILING | 1203 ms | Container 'sonarr' is stopped / not created. |
| **Radarr Movie Manager** | 7878 | radarr | AUTOMATION | **YES** | FAILING | 1207 ms | Container 'radarr' is stopped / not created. |
| **Prowlarr Indexer** | 9696 | prowlarr | INDEXER | **YES** | FAILING | 1202 ms | Container 'prowlarr' is stopped / not created. |
| **Bazarr Subtitles** | 6767 | bazarr | SUBTITLES | **YES** | FAILING | 1204 ms | Container 'bazarr' is stopped / not created. |
| **Jellyseerr Requests** | 5055 | jellyseerr | REQUESTS | **YES** | FAILING | 1202 ms | Container 'jellyseerr' is stopped / not created. |
| **Transmission Web UI** | 9091 | transmission | TORRENT | **YES** | FAILING | 1213 ms | Container 'transmission' is stopped / not created. |
| **Transmission Peer TCP** | 51413 | transmission | TORRENT | No | STANDBY | 1200 ms | Standby / optional node service not currently active. |
| **TVHeadend Web UI** | 9981 | tvheadend | LIVETV | **YES** | FAILING | 1201 ms | Container 'tvheadend' is stopped / not created. |
| **TVHeadend HTSP Stream** | 9982 | tvheadend | LIVETV | No | STANDBY | 1213 ms | Standby / optional node service not currently active. |
| **Mediastack DB GUI** | 8080 | mediastack-db | DATABASE | **YES** | FAILING | 1207 ms | Container 'mediastack-db' is stopped / not created. |
...
| **MusicBrainz Secondary** | 5001 | musicbrainz | METADATA | No | STANDBY | 1207 ms | Standby / optional node service not currently active. |
...
| :--- | :--- | :--- | :--- | :--- |
| **Dashboard Ingress** | http://voltairedeux.local | 0 WARN | 7 ms | Cannot find type [System.Net.Http.HttpClientHandler]: verify that the assembly containing this type is loaded. |
...
*Report archived in: C:\Users\waltd\OneDrive\Mediastack\handoffs\Proxy_Port_Diagnostic_Report_20260830_171904.md*
